#!/usr/bin/env python3
"""Daily Top-10 forecast — pre-open prediction for CN A-shares + US equities + funds.

Trains three LightGBM models (CN fundamentals, US price-volume, fund NAV price-volume),
predicts the latest available day for every instrument, merges by score and emits an
HTML report of the top 10 most-bullish names.

Usage:
    DATABASE_URL=postgresql+psycopg://quant:quant@localhost:5432/quant \
        python scripts/daily_top10_forecast.py [--out PATH]
...
def _train_model(b, universe, features, model_id, fund_ctx=None):
    """Train a LightGBM classification model on the trailing ~2y window."""
    end = date.today()
    ...
    if not rows:
        return None, 0
    trainer = LightGBMTrainer(features, HORIZON, task="classification",
                              num_boost_round=100)
...
def _latest_forecast(b, model, universe, features, fund_ctx=None):
    """Predict the latest available day for every instrument in universe."""
    end = date.today()
    ...
    for fid in universe:
        ...
        try:
            pred = model.predict_one(latest.features)
        except Exception:
            continue
        last_bar = bars[-1]
        out.append({
...
def _render_html(top10, meta, out_path):
    ...
    for i, f in enumerate(top10, 1):
        score = f["score"]
        pct = f"{score*100:.1f}%"
        # Chinese convention: red = bullish, green = bearish
        color = "#d32f2f" if score >= 0.55 else ("#2e7d32" if score < 0.45 else "#757575")
        tag = "看涨" if score >= 0.55 else ("看跌" if score < 0.45 else "中性")
        rows_html.append(
            f"<tr><td>{i}</td><td><b>{html.escape(f['short'])}</b><br>"
            f"<span class='sub'>{html.escape(f['name'])}</span></td>"
            f"<td>{f['market']}</td>"
            f"<td class='score' style='color:{color}'>{pct}</td>"
            f"<td style='color:{color}'>{tag}</td>"
            f"<td>{f['as_of']}</td>"
            f"<td>{f['last_close']:.4f}</td></tr>")
    models_html = "".join(
        f"<li><b>{m['id']}</b> — train {m['rows']} rows, features {m['n_feat']}</li>"
        for m in meta["models"])
    doc = f"""<!DOCTYPE html>
<html lang="zh"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>每日 Top-10 预测 — {when[:10]}</title>
<style>
...
<h1>🦫 每日 Top-10 预测</h1>
<div class="meta">生成于 {when} · horizon {HORIZON}d · 三模型融合排序</div>
<table>
<tr><th>#</th><th>标的</th><th>市场</th><th>看涨概率</th><th>信号</th><th>数据日</th><th>最新收盘</th></tr>
{"".join(rows_html)}
</table>
<div class="foot">
<b>模型：</b><ul>{models_html}</ul>
<b>预测窗口：</b>未来 {HORIZON} 个交易日方向（分类概率，非收益承诺）。
<b>排序：</b>CN A股 + 美股 + 持仓基金 三 universe 预测合并，按看涨概率降序取前 10。
</div>
<div class="disclaimer">⚠️ 量化预测仅供研究参考，不构成投资建议。模型基于历史数据，可能因市场结构变化失效。
概率为方向判断，非收益保证。请结合风控与基本面独立决策。</div>
</div></body></html>"""
...
def main() -> int:
    ...
    print(f"Daily Top-10 forecast — as_of {end} (market={args.market})")

    all_fc = []
    models_meta = []

    if args.market in ("all", "cn"):
        print("  CN A-shares: training...")
        m, n = _train_model(b, CN_A_EQUITIES, CN_FEATURES, "cn_equity_fund20d_v1", fund_ctx)
        if m:
            fc = _latest_forecast(b, m, CN_A_EQUITIES, CN_FEATURES, fund_ctx)
            for f in fc:
                f["model"] = "cn_equity_fund20d_v1"
            all_fc.extend(fc)
            models_meta.append({"id": "cn_equity_fund20d_v1", "rows": n, "n_feat": len(CN_FEATURES)})
            print(f"    trained {n} rows → {len(fc)} forecasts")

    if args.market in ("all", "us"):
        print("  US equities: training...")
        m, n = _train_model(b, US_EQUITIES, PV_FEATURES, "us_equity_pv20d_v1")
        if m:
            fc = _latest_forecast(b, m, US_EQUITIES, PV_FEATURES)
            for f in fc:
                f["model"] = "us_equity_pv20d_v1"
            all_fc.extend(fc)
            models_meta.append({"id": "us_equity_pv20d_v1", "rows": n, "n_feat": len(PV_FEATURES)})
            print(f"    trained {n} rows → {len(fc)} forecasts")

    if args.market in ("all", "fund"):
        print("  Funds: training...")
        m, n = _train_model(b, FUNDS, PV_FEATURES, "fund_nav_pv20d_v1")
        if m:
            fc = _latest_forecast(b, m, FUNDS, PV_FEATURES)
            for f in fc:
                f["model"] = "fund_nav_pv20d_v1"
            all_fc.extend(fc)
            models_meta.append({"id": "fund_nav_pv20d_v1", "rows": n, "n_feat": len(PV_FEATURES)})
            print(f"    trained {n} rows → {len(fc)} forecasts")
    ...
    all_fc.sort(key=lambda x: x["score"], reverse=True)
    top10 = all_fc[:10]

    print("\n" + "=" * 60)
    print(f"TOP 10 (of {len(all_fc)} forecasts)")
    print("=" * 60)
    for i, f in enumerate(top10, 1):
        print(f"  {i:2d}. {f['short']:8s} {f['name'][:24]:24s} "
              f"{f['score']*100:5.1f}%  [{f['market']}]  as_of={f['as_of']}")
